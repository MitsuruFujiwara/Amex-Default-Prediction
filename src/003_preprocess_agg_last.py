...
def get_features(df):
    ...
    # aggregate
    print('aggregate...')
    df_num_agg = df.groupby("customer_ID")[NUM_COLS].agg(['last'])
    df_cat_agg = df.groupby("customer_ID")[CAT_COLS].agg(['last'])
...
def main():
    ...
    sub_df = sub_df.groupby("customer_ID")[['pred_no_agg','pred_no_agg_diff']].agg(['last'])
    ...
    oof_df = oof_df.groupby("customer_ID")[['pred_no_agg','pred_no_agg_diff']].agg(['last'])
    ...
    to_json(features_json,'../configs/003_all_features_agg_last.json')
